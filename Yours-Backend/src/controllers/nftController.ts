...
import { deployAptosNFT, mintAptosNFT } from '../contract/Aptos/aptosContract';
import {
  auroraProvider,
...
const sendAuthMailForNft = async (
  ...
  try {
    ...
    //* 인증 메일 발송
    await sendMail(encodeURIComponent(code), email);
    return res
      .status(statusCode.OK)
...
const verifyMailForNft = async (
  ...
  try {
    ...
    await nftService.finishLoading(+userInfo.nftId);
    next(error);
  }
};
  ...
  try {
    ...
    if (checkNftCreator) {
      ...
      switch (nftInfo.chainType) {
        ...
        case 'Aptos': {
          await nftService.startLoading(+nftId);
          const nftData = await nftService.findNftByName(nftInfo.nftName!);
          const nftCode = nftData.length + 1;
          const data = await deployAptosNFT(
            nftInfo.nftName!+` #${nftCode}`,
            nftInfoIpfs,
            benefitInfoIpfs,
          );
  ...
  try {
    ...
    switch (nftInfo.chainType) {
      ...
      case 'Aurora': {
        ...
        const data = await createAuroraTransferOp(
          nftContract,
          mintId as number,
          contractAddress as string,
          walletAddress as string,
